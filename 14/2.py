...
while True:
  ...
  if y1 + 1 ==  MAX_Y + 2: # we're on the floor
    blocked[(x1, y1 + 1)] = ROCK
    blocked[(x1 - 1, y1 + 1)] = ROCK
    blocked[(x1 + 1, y1 + 1)] = ROCK
...
# set floor as rock far as we can see
for x in range(MIN_X, MAX_X + 1):
  blocked[(x, MAX_Y)] = ROCK
